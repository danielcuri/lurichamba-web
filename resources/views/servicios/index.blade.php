            ...
            <div class="col-12">
                ...
                <div class="breadcrumb-wrapper">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item"><a href="{{ route('servicio.index') }}"> Servicios</a>
                        </li>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <a href="#" class="btn-modal-foto" data-bs-toggle="modal"
                                                    data-bs-target="#fotoModal" data-id="{{ $servicio->id }}"
                                                    data-foto="{{ Storage::url($servicio->icono_url) ?? 'https://marketplace.canva.com/EAFbDgiv63c/1/0/1600w/canva-certificado-taller-de-administraci%C3%B3n-simple-azul-y-amarillo-Y-NyNpF3uqE.jpg' }}">
                                                    {{ $servicio->nombres ?? 'No existe descripción.....' }}
                                                </a>
                                            </td>
                                            <td>
                                                {{$servicio->tipoServicio->nombres}}
                                            </td>
                                            <td>
                                                <button
                                                    class="btn {{ $servicio->estado == '1' ? 'btn-success' : 'btn-danger' }}">
                                                    {{ $servicio->estado == '1' ? 'Habilitado' : 'Deshabilitado' }}
                                            ...
                                            <td>
                                                ...
                                                <form action="{{ route('servicio.destroy', $servicio) }}" method="POST"
                                                    ...
                                                    <a href="#" class="btn btn-primary dt-button create-new"
                                                        ...
                                                        data-id="{{ $servicio->slug }}"
                                                        data-nombres="{{ $servicio->nombres }}"
                                                        data-tipo-servicio="{{ $servicio->tipo_servicio_id }}"
                                                    ...
                                                    {{-- @can('areas.destroy') --}}
                                                    <button type="submit"
                                                        class="btn btn-danger {{ $servicio->estado == '2' ? 'disabled' : '' }}">
                                            ...
                                            <td class="text-center">
                                                <ul class="table-controls">
                                                    <form action="{{ route('servicio.activar', $servicio) }}" method="POST"
                                                        class="activar">
                                                        @csrf
                                                        @method('POST')
                                                        <button type="submit"
                                                            class="btn btn-success {{ $servicio->estado == '1' ? 'disabled' : '' }}"
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6 mt-1">
                                ...
                                <div id="result-info" class="dataTables_info">
                                    Mostrando {{ $servicios->firstItem() }} a {{ $servicios->lastItem() }} de
                            ...
                            <div class="col-md-6">
                                <div id="result-pagination" class="dataTables_paginate">
                                    <ul class="pagination justify-content-end">
                                        <li class="page-item {{ $servicios->previousPageUrl() ? '' : 'disabled' }}">
                                            <a class="page-link" href="{{ $servicios->previousPageUrl() }}">Anterior</a>
                                        </li>

                                        @if ($servicios->currentPage() > 3)
                                            <li class="page-item">
                                                <a class="page-link" href="{{ $servicios->url(1) }}">1</a>
                                            </li>
                                            @if ($servicios->currentPage() > 4)
                                        ...
                                        @foreach ($servicios->getUrlRange(max($servicios->currentPage() - 2, 1), min($servicios->currentPage() + 2, $servicios->lastPage())) as $page => $url)
                                            <li
                                                class="page-item {{ $page == $servicios->currentPage() ? 'active' : '' }}">
                                                <a class="page-link" href="{{ $url }}">{{ $page }}</a>
                                            </li>
                                        @endforeach

                                        @if ($servicios->currentPage() < $servicios->lastPage() - 2)
                                            @if ($servicios->currentPage() < $servicios->lastPage() - 3)
                                                <li class="page-item disabled">
                                                    <span class="page-link">...</span>
                                                </li>
                                            @endif
                                            <li class="page-item">
                                                <a class="page-link"
                                                    href="{{ $servicios->url($servicios->lastPage()) }}">{{ $servicios->lastPage() }}</a>
                                            </li>
                                        @endif

                                        <li class="page-item {{ $servicios->nextPageUrl() ? '' : 'disabled' }}">
            ...
            <div class="modal-dialog modal-lg modal-dialog-centered modal-add-event">
                <div class="modal-content">
                    ...
                    <div class="modal-body pb-5 px-sm-5 pt-50">
                        ...
                        @endif
                        <form id="editAddForm" class="row gy-1 pt-75 formguardar" action="{{ route('servicio.store') }}"
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body pb-5 px-sm-5 pt-50">
                        ...
                        <form id="formActualizarImagen" class="row gy-1 pt-75 actualizarFoto"
                            action="{{ route('servicio.actualizarImagen', 2) }}" method="POST"
    ...
    <script>
        $(document).ready(function() {
            ...
            $('.dt-button.create-new').click(function() {
                ...
                var actionUrl = "{{ route('servicio.update', ':id') }}";
            ...
            $('.imagen-actualizar').click(function() {
                var id = $(this).data('id');
                var actionUrl = "{{ route('servicio.actualizarImagen', ':id') }}";
                actionUrl = actionUrl.replace(':id', id);
