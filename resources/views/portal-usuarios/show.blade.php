        ...
        <div class="row">
                    ...
                    <div class="card-body">
                        ...
                        <div class="info-container">
                            ...
                            <div class="d-flex justify-content-center pt-2">
                                ...
                                @if ($persona->estado_proceso_id == 1)
                                    <form class="permiso"
                                        action="{{ route('usuario-registrado.validarProcesoPersona', $persona) }}"
                                        method="POST">
                                        @csrf
                                        @method('PUT')
                                ...
                                @else
                                    <form action="{{ route('usuario-registrado.rechazarProcesoPersona', $persona) }}"
                                        ...
                                        <button class="btn btn-outline-danger cancel-subscription mt-1 waves-effect"
                                            {{ $persona->estado_proceso_id == 1 ? 'disabled' : '' }}>Rechazar</button>

                                    </form>
                                @endif

                                {{-- <a href="javascript:;" class="btn btn-outline-danger suspend-user waves-effect">Rechazar</a> --}}
            ...
            <div class="col-xl-8 col-lg-7 col-md-7 order-0 order-md-1">
                                                    ...
                                                    <tbody>
                                                        @foreach ($persona->documentos as $documento)
                                                            <tr class="odd">
                                                                ...
                                                                <td>
                                                                    <button
                                                                        class="btn {{ $documento->estado == '1' ? 'btn-success' : 'btn-danger' }}">
                                                                        {{ $documento->estado == '1' ? 'Habilitado' : 'Deshabilitado' }}

                                                                    </button>
                                                                </td>
                                                                <td>
                                                                    @if ($documento->estado_proceso_id == 1)
                                                                        <form
                                                                            action="{{ route('usuario-registrado.validarProcesoDocumento', $documento) }}"
                ...
                <div class="card">
                    ...
                    {{-- <h4 class="card-header">Permiso Habilitados  </h4> --}}

                    <div class="card-header">
                        @if ($persona->estado_proceso_id > 2)
                            <div class="col-md-12">
                                <span
                                    class="badge badge-glow bg-success mr-2 me-1 mt-1">{{ $persona->estadosProceso->nombres }}</span>
                            </div>
                        @else
                        @endif
                    ...
                    <div class="card-body">
                        ...
                        <div class="col-md-8">
                            ...
                            <select name="tipo_utilidad_id" class="form-select" id="tipo_utilidad_id">
                                ...
                                @foreach ($tipo_utilidades as $tipo_utilidad)
                                    <option value="{{ $tipo_utilidad->id }}"
                                        {{ $persona->tipo_utilidad_id == $tipo_utilidad->id ? 'selected' : '' }}>
                                        {{ $tipo_utilidad->nombres }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        @foreach ($roles as $rol)
                            <div class="form-group">
                                <label>
                                    {!! Form::checkbox('roles[]', $rol->id, null, ['class' => 'mr-1 mt-1']) !!}
                                    {{ $rol->name }}
                                </label>
                            </div>
                        @endforeach
